...
run "integration_test_complete_bucket" {
  ...
  variables {
    bucket_name        = "integration-test-bucket-complete-${run.timestamp}"
  ...
  assert {
    condition     = aws_s3_bucket_public_access_block.this.bucket != null
    error_message = "Public access block should be created"
  }

  assert {
    condition     = aws_s3_bucket_versioning.this.bucket != null
    error_message = "Versioning configuration should be created"
  }

  assert {
    condition     = aws_s3_bucket_server_side_encryption_configuration.this.bucket != null
...
run "integration_test_minimal_bucket" {
  ...
  variables {
    bucket_name = "integration-test-bucket-minimal-${run.timestamp}"
  }
...
run "integration_test_bucket_with_kms" {
  ...
  variables {
    bucket_name = "integration-test-bucket-kms-${run.timestamp}"
...
run "integration_test_disabled_lifecycle" {
  ...
  variables {
    bucket_name = "integration-test-bucket-no-lifecycle-${run.timestamp}"
    lifecycle_rules = [
      {
        id              = "disabled-rule"
        enabled         = false
